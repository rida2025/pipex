...
/*   Updated: 2024/01/12 15:32:38 by mel-jira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	first_cmd(char **argv, char **paths, int *fd, t_list *info)
{
	...
	if (info->pid == 0)
	{
		if (!info->allcmd || !info->cmd || !info->cmdpath)
			return (ft_putstr("start No such f or directory\n"), exit(1));
		if (dup2(info->f, 0) < 0)
			exit(0);
		close(info->f);
		dup2(fd[1], 1);
...
void	midle_cmd(char **argv, char **paths, t_list *info)
{
	...
	{
		if (!info->allcmd || !info->cmd || !info->cmdpath)
			return (ft_putstr("midle No such f or directory\n"), exit(1));
...
void	last_cmd(char **argv, char **paths, t_list *info)
{
	...
	{
		if (!info->allcmd || !info->cmd || !info->cmdpath)
			return (ft_putstr("last No such f or directory\n"), exit(1));
		if (info->here_doc == 1)
			short_here_doc_line(&info, argv);
		else if (info->here_doc == 0)
			short_normal_line(&info, argv);
		if (info->f < 0)
			return (write(2, "open fail to open last f\n", 29), exit(1));
